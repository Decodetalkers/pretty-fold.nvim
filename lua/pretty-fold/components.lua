...
function M.content(config)
    ...
    if not cache.comment_tokens then
        ...
        -- Trim redundant spaces from the beginning and the end if any.
    ...
    if not cache.regex then
        ...
        -- List of regexes for seeking all comment tokens at the beginning of the line.
    ...
    if not cache.lua_patterns then
        ...
        -- Comment token at the beginning of the line
        ...
        for _, token in ipairs(comment_tokens.escaped) do
            ...
            table.insert(cache.lua_patterns.str_with_only_comment_token, table.concat({ "^%s*", token, "%s*$" }))
            table.insert(cache.lua_patterns.comment_token_at_start, table.concat({ "^", token, "%s*" }))
            table.insert(cache.lua_patterns.comment_token_at_eol, table.concat({ "%s*", token, "%s*$" }))
            table.insert(cache.lua_patterns.comment_substring_at_eol, table.concat({ "%s*", token, ".*$" }))
        end
    end
    ...
    -- Add matchup pattern
    if
        type(config.add_close_pattern) == "boolean" -- Add matchup pattern
        ...
        for _, pat in ipairs(config.matchup_patterns) do
            ...
            if num_op > 0 then
                ...
                while stop do
                    ...
                    -- If number of closing patterns become equal to number of opening
                    -- patterns, then break.
                    if #found - num_op == num_op then
                        break
                    end
                end
            end
            ...
            for _, f in ipairs(found) do
                table.insert(found_patterns, { pat = pat, pos = f.start, oppening = f.oppening })
            end
        end
